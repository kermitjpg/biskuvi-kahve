...
if ($_SESSION["yetki"] != "1") {
    ...
    echo "<script> Swal.fire( {title: 'Hata!', text:'Yetkisiz kullanıcı', icon:'error', confirmButtonText:'Kapat', confirmButtonColor: '#000', }).then((value)=>{
            if(value.isConfirmed){window.location.href='iletisim.php'}
        })</script>";
    exit;
}


$sorgu = $baglanti->prepare("select * from iletisimbaslik WHERE id=:id");
$sorgu->execute(['id' => (int)$_GET["id"]]);
$sonuc = $sorgu->fetch();

if ($_POST) { ///veri güncelle

    $guncelleSorgu = $baglanti->prepare("Update iletisimbaslik set ustBaslik=:ustBaslik");
    ...
    if ($guncelle) {
        ...
        echo "<script> Swal.fire( {title: 'Başarılı!', text:'Güncelleme başarılı!', icon:'success', confirmButtonText:'Kapat', confirmButtonColor: '#000', }).then((value)=>{
            if(value.isConfirmed){window.location.href='iletisimbaslik.php'}
        })</script>";
    }
}
?>
